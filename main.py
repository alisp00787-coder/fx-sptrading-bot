...
🔴 قانون مطلق و غیرقابل نقض زبان:
- تمام پاسخ‌هات باید ۱۰۰٪ فارسی روان و طبیعی باشه
...
۱. حتماً بین هر بخش یک خط خالی بذار (Enter دو بار)
...
۷. بعد از هر عنوان، Enter بزن و بعد متن رو شروع کن
...
⚠️ نکته ایمنی:
اگر کاربر نشانه بحران روانی نشون داد، به متخصص ارجاع بده."""
...
def _clean_persian(text: str) -> str:
    """پاکسازی متن با حفظ خط‌های جدید و حذف کلمات غیرفارسی"""
    
    # 🔥 لیست کلمات شایع غیرفارسی که باید جایگزین بشن
    replacements = {
        'thường': 'معمولاً',
        'often': 'اغلب',
        'usually': 'معمولاً',
    ...
    for foreign, persian in replacements.items():
        text = re.sub(r'\b' + foreign + r'\b', persian, text, flags=re.IGNORECASE)
    
    # 🔥 حذف کاراکترهای ویتنامی، چینی، ژاپنی، کره‌ای، هندی، تایلندی
    text = re.sub(r'[\u0E00-\u0E7F]', '', text)  # تایلندی
    text = re.sub(r'[\u4E00-\u9FFF]', '', text)  # چینی
    text = re.sub(r'[\u3040-\u309F]', '', text)  # هیراگانا
    text = re.sub(r'[\u30A0-\u30FF]', '', text)  # کاتاکانا
    text = re.sub(r'[\uAC00-\uD7AF]', '', text)  # کره‌ای
    text = re.sub(r'[\u0900-\u097F]', '', text)  # هندی
    
    # 🔥 حذف حروف ویتنامی با علائم خاص
    ...
    # 🔥 حذف چند فاصله پشت هم (ولی خط جدید رو حفظ کن!)
    text = re.sub(r'[ \t]+', ' ', text)
    
    # 🔥 حذف فاصله اضافی اول و آخر هر خط
    lines = text.split('\n')
    lines = [line.strip() for line in lines]
    text = '\n'.join(lines)
    
    # 🔥 حذف بیش از ۲ خط خالی پشت هم
    text = re.sub(r'\n{3,}', '\n\n', text)
